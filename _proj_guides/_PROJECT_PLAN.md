...
- **Corpus:** ~4194 PubMed abstracts (title + abstract + PMID), stored in a text file (`filtered_pubmed_abstracts.txt`), indexed in OpenSearch.
...
Using the selected reference sentences (top-3 per doc, top-k docs from Phase 1 retrieval — k is a configurable argument), we prompt **GPT-4o** (via the IAedu API) to generate a **≤250-word answer** where each sentence is attributed to ≤3 PMIDs from the valid corpus. The prompt enforces factual grounding and citation format. A zero-shot prompt is used initially, with the flexibility to switch to few-shot if needed. vLLM at `amalia.novasearch.org` is available as a secondary generator for comparison experiments.
...
- **Sentence alignment:** Is each selected reference sentence relevant to the query? Labels: `Required`, `Unnecessary`, `Borderline`, `Inappropriate` (as per TREC BioGen rubric).
- **Answer entailment:** Is the generated answer fully entailed by its cited reference sentences? Labels: `Supported`, `Partially Supported`, `Unsupported` (as per TREC BioGen rubric).

We start with the exact prompts from the project guide and then manually verify and adapt them on a 5–10 example sample to calibrate for the biomedical domain before running at scale.
...
| Evaluation library      | Custom Python/numpy metrics (`src/evaluation/metrics.py`)                              | Lab03 pattern; P@k, R@k, AP, MAP, MRR, NDCG implemented from scratch             |
...
| Generation LLM          | GPT-4o via IAedu API (primary); vLLM at `amalia.novasearch.org` as optional comparison | User preference — frontier quality for both generation and judging               |
| Judge LLM               | GPT-4o via IAedu API                                                                   | Frontier capability needed for entailment judgement                              |
| Query field             | `topic + question` combined (locked from Phase 1 tuning)                               | Best nDCG@100 in Phase 1 query field ablation; narrative didn't help             |
| Sentence splitter       | NLTK `sent_tokenize` (punkt)                                                           | Simple, fast, adequate for PubMed; scispaCy noted as improvement in report       |
| Generation prompt style | Zero-shot RAG prompt (flexible to switch to few-shot later)                            | Start simple, make it easy to add examples later if needed                       |
