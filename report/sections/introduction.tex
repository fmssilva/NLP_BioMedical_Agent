...
\textbf{Phase~1} (the focus of this report) builds and evaluates a complete
...
\textbf{Phase~2} (upcoming) will add cross-encoder reranking of retrieved
evidence, LLM-generated paragraph answers with per-sentence citations, and
...
\noindent
\textbf{Contribution.}
We set up a comprehensive fine-tuning and grid-search pipeline to identify the
best retrieval models for biomedical evidence retrieval.  Each parameter
configuration is materialised as a dedicated field in the OpenSearch index,
enabling any downstream application to issue a query and receive the
highest-quality ranked list of related PubMed abstracts for further analysis
or generation.
